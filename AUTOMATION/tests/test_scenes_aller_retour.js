...
const idDe = s => s.startsWith('#') ? s.slice(1).split(' ')[0] : s;
let QSA = () => [];                                   // seam swappe par le test
globalThis.window = globalThis;
globalThis.document = {
  querySelector(s){
    if (/\s/.test(s)) return null;
    return NOEUDS.get(idDe(s)) || faire(idDe(s));
  },
  querySelectorAll(s){ return QSA(s); },
  addEventListener(){},
  createElement(){
    return {className: '', innerHTML: '', dataset: {}, style: {}, enfants: [],
            addEventListener(){}, querySelectorAll(){ return []; },
            querySelector(){ return {}; },
            append(el){ this.enfants.push(el); }};
  },
...
(async () => {
  ...
  function peindreEtRelire(){
    ...
    QSA = s => s === '#sceneCards .sceneCard' ? cartes : [];
    return cartes;
  }
